...
- **HLS** (`.m3u8`) - Uses **hls.js 1.7.2** on Chrome / Firefox / Edge / desktop Safari (CDN fallback when not preloaded). On iOS / iPadOS the native `<video>` HLS support is used and bridged into VidPly's caption / transcript / quality UI via the `TextTrack` API.
