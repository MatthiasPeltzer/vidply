...
- Pin the loaded versions of `hls.js` / `dash.js` via the `hlsScriptUrl` / `dashScriptUrl` options and add Subresource Integrity hashes (the library adds them automatically when the URL ends in `.js`). VidPly defaults to **hls.js 1.7.3** and **dash.js 5.2.1** (modern UMD).
