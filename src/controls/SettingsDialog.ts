...
export class SettingsDialog {
  ...
  createSpeedSettings() {
    ...
    select.addEventListener('change', (e) => {
      this.player.setPlaybackSpeed(parseFloat((e.target as HTMLSelectElement).value));
    });
  ...
  createCaptionStyleControl(property: string, label: string, options: Array<{ label: string; value: string }>) {
  ...
  createColorControl(property: string, label: string) {
    const wrapper = DOMUtils.createElement('div', {
      className: `${this.player.options.classPrefix}-settings-control`
    });
  ...
  createRangeControl(property: string, label: string, min: number, max: number, step: number) {
    const wrapper = DOMUtils.createElement('div', {
      className: `${this.player.options.classPrefix}-settings-control`
    });
