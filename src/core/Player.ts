...
export class Player extends EventEmitter<PlayerEventMap> {
    ...
    _managersLoading: any;
    _originalBodyBackground: any;
    _originalBodyHeight: any;
    _originalBodyOverflow: any;
    _originalBodyPosition: any;
    _originalBodyWidth: any;
    _originalElement: any;
    _originalHtmlBackground: any;
    _originalHtmlOverflow: any;
    _originalScrollX: any;
    _originalScrollY: any;
    _originalViewport: any;
    _pendingSource: any;
    _resumeChecked: boolean | undefined;
    _saveProgressThrottled: any;
    _sourceElementsCache: any;
    _sourceElementsDirty: any;
    _switchingRenderer: boolean | undefined;
    _trackElementsCache: any;
    _trackElementsDirty: any;
    _textTracksCache: any;
    _textTracksDirty: boolean | undefined;
    audioDescriptionCaptionTracks: any;
    audioDescriptionSourceElement: any;
    audioDescriptionSrc: any;
    currentSignLanguage: any;
    currentSource: any;
    debouncedPositionPlayOverlay: any;
    fullscreenChangeHandler: any;
    metadataAlertHandlers: any;
    metadataCueChangeHandler: any;
    noticeElement: any;
    noticeTimeout: any;
    orientationHandler: any;
    orientationQuery: any;
    originalAudioDescriptionSource: any;
    originalSrc: any;
    playButtonOverlay: any;
    resizeHandler: any;
    resizeObserver: any;
    resumePromptElement: any;
    signLanguageCustomKeyHandler: any;
    signLanguageDesiredPosition: any;
    signLanguageDocumentClickHandler: any;
    signLanguageDocumentClickHandlerAdded: any;
    signLanguageDraggable: any;
    signLanguageDragOptionButton: any;
    signLanguageDragOptionText: any;
    signLanguageHeaderKeyHandler: any;
    signLanguageHeader: any;
    signLanguageHandlers: any;
    signLanguageInteractionHandlers: any;
    signLanguageResizeHandles: any;
    signLanguageResizeOptionButton: any;
    signLanguageSelector: any;
    signLanguageSettingsButton: any;
    signLanguageSettingsHandlers: any;
    signLanguageSettingsMenu: any;
    signLanguageSettingsMenuVisible: any;
    signLanguageSettingsMenuJustOpened: any;
    signLanguageSettingsMenuKeyHandler: any;
    signLanguageResizeOptionText: any;
    signLanguageSources: any;
    signLanguageSrc: any;
    signLanguageVideo: any;
    signLanguageWrapper: any;
    timeouts: any;
     ...
     */
    showNotice(message: any, { timeout = 2500, priority = 'polite' } = {}) {
        try {
            if (!message) return;
    ...
    async ensureTranscriptManager() {
        ...
        const Manager = module.TranscriptManager || (module as any).default;
    ...
    async initFeatureManagers() {
        ...
        const hasSourceElementsWithDesc = this.sourceElements.some(
            (el: any) => el.getAttribute('data-desc-src') || el.getAttribute('data-orig-src')
        );
    ...
    detectHtmlLanguage() {
        ...
        if ((i18n as any).builtInLanguageLoaders && (i18n as any).builtInLanguageLoaders[normalizedLang]) {
            return normalizedLang;
        }
    ...
    getVideoId() {
        ...
        // Get source URL
        let src: any = this.element.src;
        if (!src) {
            const sourceEl: any = this.element.querySelector('source');
            src = sourceEl?.src;
        }
     ...
     */
    _formatResumeTime(seconds: any) {
        const h = Math.floor(seconds / 3600);
        const m = Math.floor((seconds % 3600) / 60);
        const s: any = Math.floor(seconds % 60);
     ...
     */
    showResumePrompt(savedTime: any) {
     ...
     */
    setTheme(themeName: any, customVariables = {}) {
    ...
    async initializeRenderer() {
        let src: any = this._pendingSource;
        let rendererClass = null;

        if (!src) {
            const sourceElements: any = Array.from(this.element.querySelectorAll('source'));
    ...
    _selectBestSource(sourceElements: any) {
        ...
        if (hasMSE) {
            chosen = sources.find((s: any) => s.src.includes('.mpd'));
        }

        if (!chosen) {
            const hlsSource = sources.find((s: any) => s.src.includes('.m3u8'));
        ...
        if (!chosen) {
            chosen = sources.find((s: any) => !s.src.includes('.mpd') && !s.src.includes('.m3u8')) || sources[0];
        }

        const fallbacks = sources
            .filter((s: any) => s !== chosen)
            .map((s: any) => ({ src: s.src, type: s.type }));
     ...
     */
    findTextTrack(kind: any, language: any = null) {
        const tracks: any = this.textTracks;
        if (language) {
            return tracks.find((t: any) => t.kind === kind && t.language === language);
        }
        return tracks.find((t: any) => t.kind === kind);
    }
     ...
     */
    findSourceElement(attribute: any, value: any = null) {
        const sources = this.sourceElements;
        if (value) {
            return sources.find((el: any) => el.getAttribute(attribute) === value);
        }
        return sources.find((el: any) => el.hasAttribute(attribute));
    }
     ...
     */
    findTrackElement(track: any) {
        return this.trackElements.find((el: any) => el.track === track);
    }
     ...
     */
    resolvePosterPath(posterPath: any) {
        if (!posterPath) {
            return posterPath;
        }
     ...
     */
    setManagedTimeout(callback: any, delay: any) {
     ...
     */
    clearManagedTimeout(timeoutId: any) {
        if (timeoutId) {
    ...
    async load(config: any) {
        try {
            ...
            existingTracks.forEach((track: any) => track.remove());
            this.invalidateTrackCache();
            ...
            if (!isExternalRenderer) {
                ...
                if (config.type) {
                    (this.element as any).type = config.type;
                }
            } else {
            ...
            if (config.tracks && config.tracks.length > 0) {
                config.tracks.forEach((trackConfig: any) => {
    ...
    enterFullscreen() {
        ...
        if (fullscreenPromise && fullscreenPromise.catch) {
            fullscreenPromise.catch((err: any) => {
        ...
        if (!elem.requestFullscreen && !(elem as any).webkitRequestFullscreen && 
            !(elem as any).mozRequestFullScreen && !(elem as any).msRequestFullscreen) {
            this._enablePseudoFullscreen();
        } else {
    ...
    _restoreBackgroundInteractivity() {
        if (this._inertElements) {
            this._inertElements.forEach((el: any) => {
     ...
     */
    findMatchingCaptionTime(targetText: any, tracks: any) {
        ...
        for (const trackInfo of tracks as any[]) {
            if (trackInfo.kind !== 'captions' && trackInfo.kind !== 'subtitles') {
                continue;
            }

            const track: any = trackInfo.track;
            if (!track || !track.cues) {
                continue;
            }
            ...
            for (let i = 0; i < track.cues.length; i++) {
                ...
                const targetWords = normalizedTarget.split(/\s+/).filter((w: any) => w.length > 2);
                const cueWords = cueText.split(/\s+/).filter((w: any) => w.length > 2);
                
                if (targetWords.length > 0 && cueWords.length > 0) {
                    const matchingWords = targetWords.filter((word: any) => cueWords.includes(word));
    ...
    async _legacyEnableAudioDescription() {
        ...
        const hasSourceElementsWithDesc = this.sourceElements.some((el: any) => el.getAttribute('data-desc-src'));
        ...
        if (this.captionManager && this.captionManager.currentTrack) {
            const track: any = this.captionManager.currentTrack.track;
        ...
        let swappedTracksForTranscript: any[] = [];
        ...
        if (this.audioDescriptionSourceElement) {
            ...
            const sourceElements: any = this.sourceElements;
            let sourceElementToUpdate = null;
            let descSrc = this.audioDescriptionSrc;
            
            for (const sourceEl of sourceElements as any[]) {
            ...
            if (this.audioDescriptionCaptionTracks.length > 0) {
                ...
                const validationPromises = this.audioDescriptionCaptionTracks.map(async (trackInfo: any) => {
                ...
                const tracksToSwap = validationResults.filter((result: any) => result.exists);
            ...
            const sourcesToUpdate: any[] = [];
            
            allSourceElements.forEach((sourceEl: any) => {
                const descSrcAttr = sourceEl.getAttribute('data-desc-src');
                ...
                if (descSrcAttr) {
                    ...
                    const type = sourceEl.getAttribute('type');
                    const src: any = sourceEl.getAttribute('src');
                    sourcesToUpdate.push({
                        src: src,
                        type: type,
                        origSrc: null,
            ...
            const hasSourceElementsWithDesc = fallbackSourceElements.some((el: any) => el.getAttribute('data-desc-src'));
            
            if (hasSourceElementsWithDesc) {
                const fallbackSourcesToUpdate: any[] = [];
                
                fallbackSourceElements.forEach((sourceEl: any) => {
                    const descSrcAttr = sourceEl.getAttribute('data-desc-src');
                    const currentSrc = sourceEl.getAttribute('src');
        ...
        if (swappedTracksForTranscript.length > 0 && this.captionManager) {
            ...
            if (this.captionManager.currentTrack) {
                const currentTrackIndex = this.captionManager.tracks.findIndex((t: any) => t.track === this.captionManager.currentTrack.track);
            ...
            const reloadTracks = () => {
                ...
                if (wasCaptionsEnabled && currentTrackInfo && this.captionManager.tracks.length > 0) {
                    ...
                    const matchingTrackIndex = this.captionManager.tracks.findIndex((t: any) => 
    ...
    async _legacyDisableAudioDescription() {
        ...
        if (this.audioDescriptionCaptionTracks.length > 0) {
            ...
            const tracksToRestore = this.audioDescriptionCaptionTracks.map((trackInfo: any) => {
                const trackElement = trackInfo.trackElement;
            ...
            tracksToRestore.forEach(({ trackInfo }: any) => {
            ...
            await new Promise<void>(resolve => {
                setTimeout(() => {
                    tracksToRestore.forEach(({ trackInfo, parent, nextSibling, attributes }: any) => {
                        swappedTracksForTranscript.push(trackInfo);
        ...
        const allSourceElements = this.sourceElements;
        const hasSourceElementsToSwap = allSourceElements.some((el: any) => el.getAttribute('data-orig-src'));
        
        if (hasSourceElementsToSwap) {
            const sourcesToRestore: any[] = [];
    ...
    _legacyEnableSignLanguage() {
        ...
        this.signLanguageResizeHandles = ['n', 's', 'e', 'w', 'ne', 'nw', 'se', 'sw'].map(dir => {
            const handle: any = DOMUtils.createElement('div', {
        ...
        this.signLanguageResizeHandles.forEach((handle: any) => this.signLanguageWrapper.appendChild(handle));

        // Set width FIRST to ensure proper dimensions
        const saved = this.storage.getSignLanguagePreferences() as any;
    ...
    async toggleSignLanguage() {
        ...
        if (!this.renderer && this.playlistManager && this.playlistManager.tracks?.length) {
            ...
            const result: any = manager.toggle();
    ...
    _legacySetupSignLanguageInteraction() {
        ...
        this.signLanguageDraggable = new DraggableResizable(this.signLanguageWrapper, {
            ...
            onPointerResizeToggle: (enabled) => {
                ...
                this.signLanguageResizeHandles.forEach((handle: any) => {
        ...
        this.signLanguageCustomKeyHandler = (e: any) => {
            const key: any = e.key.toLowerCase();
    ...
    getSignLanguageLabel(langCode: any) {
    ...
    switchSignLanguage(langCode: any) {
        return this.signLanguageManager?.switchLanguage(langCode);
    }

    // Legacy method preserved for reference
    _legacySwitchSignLanguage(langCode: any) {
    ...
    _legacyShowSignLanguageSettingsMenu() {
        ...
        if (!this.signLanguageDocumentClickHandlerAdded) {
            this.signLanguageDocumentClickHandler = (e: any) => {
    ...
    handleError(error: any) {
        ...
        if (this._fallbackSources && this._fallbackSources.length > 0) {
            ...
            this._fallbackToNextSource().then((success: any) => {
                if (!success) {
    ...
    log(...messages: any[]) {
        ...
        if (messages.length > 0) {
            ...
            if (typeof potentialType === 'string' && (console as any)[potentialType]) {
                type = potentialType;
        ...
        if (typeof (console as any)[type] === 'function') {
            (console as any)[type]('[VidPly]', ...messages);
        } else {
            ...
            if (metadataTrack) {
                ...
                this.metadataCueChangeHandler = () => {
                    ...
                    if (activeCues.length > 0) {
                        // Debug logging
                        if (this.options.debug) {
                            this.log('[Metadata] Active cues:', activeCues.map((c: any) => ({
                                start: c.startTime,
                                end: c.endTime,
                                text: c.text
                            })));
    ...
    cacheMetadataAlertContent(element: any, config: any = {}) {
        if (!element) {
            return;
        }
    ...
    restoreMetadataAlertContent(element: any, config: any = {}) {
        if (!element) {
            return;
        }
        const titleSelector = config.titleSelector || '[data-vidply-alert-title], h3, header';
    ...
    focusMetadataTarget(target: any, fallbackElement: any = null) {
    ...
    handleMetadataAlert(selector: string, options: any = {}) {
        if (!selector) {
            return;
        }
    ...
    handleMetadataHashtags(hashtags: any) {
        ...
        hashtags.forEach(tag => {
            const config: any = this.resolveMetadataConfig(configMap, tag);
            if (!config) {
                return;
            }
     ...
     */
    handleMetadataCue(cue: any) {
        const text: any = cue.text.trim();
